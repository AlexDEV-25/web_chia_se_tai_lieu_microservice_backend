...
//    private final FileManager fileStorage;
    ...
    public UserDetailResponse getDetailUser(Long userId) {
        UserDetail find = userRepository.findByUserId(userId)
    ...
    public UserDetailResponse save(UserDetailRequest dto) {
...
//    }
    ...
    public void changeConnectStatus(Long userId, ConnectionStatus status) {
        UserDetail find = userRepository.findByUserId(userId).orElseThrow(() -> AppException.builder().appError(AppError.USER_NOT_FOUND).build());
